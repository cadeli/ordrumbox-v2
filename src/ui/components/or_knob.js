...
export class OrKnob {
    ...
    /** Clamps and rounds a raw value to the nearest valid step. */
    _clampStep(raw) {
    ...
    _onMousedown(e) {
        ...
        const sensitivity = (this._max - this._min) / 200

        const onMove = (ev) => {
            const deltaY = this._dragStartY - ev.clientY
            if (Math.abs(deltaY) > 2) this._dragging = true
            const clamped = this._clampStep(this._dragStartVal + deltaY * sensitivity)
    ...
    _onKeydown(e) {
        ...
        const clamped = this._clampStep(this._value + delta)
        this.setValue(clamped)
        this._onChange?.(clamped, this._key)
    }
