    ...
    sendMidiNotes = (tick, atTime) => {
        ...
        notesToPlay.forEach(flatNote => {
            if (flatNote.track.mute === false) {
                ...
                if (mapping) {
                    const channel   = ((_v=>!Number.isNaN(_v)?_v:(logger.warn('Fallback','pi',mapping.ch,10),10))(parseInt(mapping.ch)))
                    const note      = ((_v=>!Number.isNaN(_v)?_v:(logger.warn('Fallback','pi',mapping.key,60),60))(parseInt(mapping.key)))
