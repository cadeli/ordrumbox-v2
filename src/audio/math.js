...
export function computeLfoValue(lfo, tick, nbTicks = TICK * 4, controlKey = null, audioTime = null, bpm = null) {
    if (!lfo) return 0
    const freqVal = ((_v=>!Number.isNaN(_v)?_v:(logger.warn('Fallback','pf',lfo.freq,1),1))(parseFloat(lfo.freq)))
    let min = ((_v=>!Number.isNaN(_v)?_v:(logger.warn('Fallback','pf',lfo.min,0),0))(parseFloat(lfo.min)))
    let max = ((_v=>!Number.isNaN(_v)?_v:(logger.warn('Fallback','pf',lfo.max,1),1))(parseFloat(lfo.max)))
    const phase = ((_v=>!Number.isNaN(_v)?_v:(logger.warn('Fallback','pf',lfo.phase,0),0))(parseFloat(lfo.phase)))
    const waveName = lfo.type || lfo.waveform || 'sine'
    let wave = Utils.waveList.indexOf(waveName)
    if (wave === -1) wave = ((_v=>!Number.isNaN(_v)?_v:(logger.warn('Fallback','pf',waveName,0),0))(parseFloat(waveName)))
