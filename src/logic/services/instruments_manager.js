...
export default class InstrumentsManager {
    static DATA = {
        "instruments": [
            ...
            { "id": "CONGAS", "midi": [{ "name": "Open Hi Conga", "key_based": true }], "subst": { "id1": "HI_CONGAS", "id2": "HI_TOM", "id3": "TOM" }, "name": { "syn": [".*CON.*", ".*CNG.*", ".*PER.*", "MC"] }, "drum": true, "pan": "0" },
            ...
            { "id": "CROMAPERC", "midi": [{ "name": "Marimba", "key_based": false }], "subst": { "id1": "PIANO", "id2": "MELO", "id3": "HI_TOM" }, "name": { "syn": [".*CROMA.*", ".*CHROMATIC.*", ".*CELESTA.*", ".*GLOCK.*", ".*MUSIC.*BOX.*", ".*VIBRAPHONE.*", ".*VIBES.*", ".*MARIMBA.*", ".*XYLO.*", ".*TUBULAR.*BELL.*", ".*DULCIMER.*"] }, "drum": false, "pan": "0" },
            { "id": "CYM", "drum": true, "pan": "0", "subst": { "id1": "CRASH" } },
            { "id": "ENSEMBLE", "midi": [{ "name": "String Ensemble 1", "key_based": false }], "subst": { "id1": "BRASS", "id2": "PIANO", "id3": "MELO" }, "name": { "syn": [".*ENSEM.*", ".*CHOIR.*", ".*VOIC.*", ".*ORCHESTRA.*HIT.*", ".*ORCH.*HIT.*"] }, "drum": false, "pan": "0" },
            { "id": "ETHNIC", "midi": [{ "name": "Sitar", "key_based": false }], "subst": { "id1": "BASS", "id2": "RIMSHOT", "id3": "HI_TOM" }, "name": { "syn": [".*ETHN.*", ".*SITAR.*", ".*BANJO.*", ".*SHAMISEN.*", ".*KOTO.*", ".*KALIMBA.*", ".*BAGPIPE.*", ".*FIDDLE.*", ".*SHANAI.*"] }, "drum": false, "pan": "0" },
            ...
            { "id": "HIT", "midi": [{ "name": "Synth Drum", "key_based": false }], "subst": { "id1": "SNARE" }, "name": { "syn": [".*BEEP.*", ".*BIP.*", ".*ZAP.*", ".*POP.*", ".*SHOT.*", ".*LASER.*", ".*GUN.*"] }, "drum": true, "pan": "5" },
            ...
            { "id": "LONGBRASS", "midi": [{ "name": "Brass Section", "key_based": false }], "subst": { "id1": "SHORTBRASS", "id2": "BRASS", "id3": "MELO" }, "name": { "syn": [".*LONG.*BRASS.*"] }, "drum": false, "pan": "0" },
            { "id": "LOOP", "midi": [{ "name": "Applause", "key_based": true }],  "name": { "syn": [".*LOOP.*"] }, "drum": false, "pan": "0" },
            ...
            { "id": "OHI_CONGAS", "midi": [{ "name": "Open Hi Conga", "key_based": true }], "subst": { "id1": "HI_CONGAS" }, "name": { "syn": [".*OHCONGA.*", ".*OPEN.*CONGA.*"] }, "drum": true, "pan": "0" },
            { "id": "ORGAN", "midi": [{ "name": "Drawbar Organ", "key_based": false }], "subst": { "id1": "PIANO", "id2": "MELO", "id3": "HI_TOM" }, "name": { "syn": [".*ORGAN.*", ".*ACCORDION.*", ".*HARMONICA.*", ".*TANGO.*"] }, "drum": false, "pan": "0" },
            ...
            { "id": "PERCUSSIVE", "midi": [{ "name": "Steel Drums", "key_based": false }], "subst": { "id1": "HI_CONGAS", "id2": "HI_TOM", "id3": "RIMSHOT" }, "name": { "syn": [".*PERCUSS.*", ".*STEEL.*DRUM.*", ".*TAIKO.*", ".*MELODIC.*TOM.*", ".*SYNTH.*DRUM.*"] }, "drum": true, "pan": "0" },
            ...
            { "id": "PIPE", "midi": [{ "name": "Flute", "key_based": false }], "subst": { "id1": "PIANO", "id2": "MELO", "id3": "HI_TOM" }, "name": { "syn": [".*PIPE.*", ".*FLUTE.*", ".*PICCOLO.*", ".*RECORDER.*", ".*PAN.*FLUTE.*", ".*PANFLUTE.*", ".*SHAKUHACHI.*", ".*OCARINA.*", ".*BLOWN.*BOTTLE.*"] }, "drum": false, "pan": "0" },
            { "id": "RAZOR", "midi": [{ "name": "Gunshot", "key_based": true }], "subst": { "id1": "HIT" }, "name": { "syn": ["RAZ", ".*RAZOR.*"] }, "drum": true, "pan": "5" },
            { "id": "REED", "midi": [{ "name": "Oboe", "key_based": false }], "subst": { "id1": "PIANO", "id2": "MELO", "id3": "HI_TOM" }, "name": { "syn": [".*REED.*", ".*OBOE.*", ".*CLARINET.*", ".*BASSOON.*", ".*ENGLISH.*HORN.*"] }, "drum": false, "pan": "0" },
            ...
            { "id": "SAX", "midi": [{ "name": "Soprano Sax", "key_based": false }], "subst": { "id1": "PIANO" }, "name": { "syn": ["saxophone", "saxo", ".*SAX.*"] }, "drum": false, "pan": "7" },
            { "id": "SCRATCH", "midi": [{ "name": "Reverse Cymbal", "key_based": false }], "subst": { "id1": "SOUNDEFFECT", "id2": "HIT", "id3": "CRASH" }, "name": { "syn": [".*SCRATCH.*"] }, "drum": true, "pan": "0" },
            ...
            { "id": "SHORTBRASS", "midi": [{ "name": "Synth Brass 2", "key_based": false }], "subst": { "id1": "LONGBRASS", "id2": "BRASS", "id3": "MELO" }, "name": { "syn": [".*SHORT.*BRASS.*"] }, "drum": false, "pan": "0" },
            { "id": "SNARE", "midi": [{ "name": "Acoustic Snare", "key_based": true },{ "name": "Electric Snare", "key_based": true }], "subst": { "id1": "SNARE" }, "name": { "syn": ["SN", ".*SNAR.*", ".*SD.*", "SN", "SNR", "ACC AC", "AC", ".*BRUSH.*"], "adj_style": "ACCOUSTIC" }, "drum": true, "pan": "3" },
            { "id": "SOUNDEFFECT", "midi": [{ "name": "Seashore", "key_based": true }], "subst": { "id1": "CRASH", "id2": "HIT", "id3": "SCRATCH" }, "name": { "syn": [".*FX.*", ".*SOUND.*", ".*SND.*", ".*VINYL.*", ".*CRACKL.*", ".*RISER.*", ".*IMPACT.*", ".*DOWNLIFT.*", ".*FOLEY.*", ".*SEASHORE.*", ".*BIRD.*", ".*APPLAUSE.*", ".*HELICOPTER.*", ".*TELEPHONE.*", ".*GUNSHOT.*", ".*REVERSE.*", ".*BREATH.*", ".*FRET.*NOISE.*", ".*RAIN.*", ".*SOUNDTRACK.*", ".*CRYSTAL.*", ".*ATMOS.*", ".*BRIGHT.*", ".*GOBLIN.*", ".*ECHO.*", ".*SCI.*FI.*"] }, "drum": true, "pan": "0" },
            { "id": "STRINGS", "midi": [{ "name": "String Ensemble 1", "key_based": false }], "subst": { "id1": "BRASS", "id2": "PIANO", "id3": "MELO" }, "name": { "syn": [".*STRING.*", ".*VIOLIN.*", ".*VIOLA.*", ".*CELLO.*", ".*CONTRABASS.*", ".*HARP.*", ".*TIMPANI.*", ".*PIZZICATO.*", ".*TREMOLO.*"] }, "drum": false, "pan": "0" },
            { "id": "SYNTH", "drum": false, "pan": "0", "subst": { "id1": "MELO" } },
            { "id": "SYNTHEFFECT", "midi": [{ "name": "Reverse Cymbal", "key_based": false }], "subst": { "id1": "HIT", "id2": "CRASH", "id3": "RIMSHOT" }, "name": { "syn": [".*SYNTH.*EFFECT.*", ".*FX.*"] }, "drum": true, "pan": "0" },
            { "id": "SYNTHLEAD", "midi": [{ "name": "Lead 1 (square)", "key_based": false }], "subst": { "id1": "PIANO", "id2": "MELO", "id3": "HI_TOM" }, "name": { "syn": [".*SYNTH.*", ".*LEAD.*", ".*SQUARE.*", ".*SAW.*", ".*CALLIOPE.*", ".*CHIFF.*", ".*CHARANG.*", ".*FIFTHS.*"] }, "drum": false, "pan": "0" },
            { "id": "SYNTHPAD", "midi": [{ "name": "Pad 1 (new age)", "key_based": false }], "subst": { "id1": "HIT", "id2": "CRASH", "id3": "RIMSHOT" }, "name": { "syn": [".*SYNTH.*PAD.*", ".*PAD.*", ".*NEW.*AGE.*", ".*WARM.*", ".*BOWED.*", ".*METALLIC.*", ".*HALO.*", ".*SWEEP.*"] }, "drum": false, "pan": "0" },
            { "id": "TAMB", "midi": [{ "name": "Tambourine", "key_based": true }], "subst": { "id1": "SNARE", "id2": "HI_TOM", "id3": "CHH" }, "name": { "syn": [".*TAMB.*"] }, "drum": true, "pan": "0" },
            { "id": "TAMBOURINE", "midi": [{ "name": "Tambourine", "key_based": true }], "subst": { "id1": "OHH" }, "name": { "syn": [".*TAMB.*"] }, "drum": true, "pan": "-2" },
            { "id": "TIMBAL", "midi": [{ "name": "High Timbale", "key_based": true }], "subst": { "id1": "LO_TOM", "id2": "CRASH", "id3": "SNARE" }, "name": { "syn": [".*TIMBAL.*", "TIMBALE"] }, "drum": true, "pan": "0" },
            ...
            { "id": "VIBRA", "midi": [{ "name": "Vibraslap", "key_based": true }], "subst": { "id1": "HIT", "id2": "CRASH", "id3": "RIMSHOT" }, "name": { "syn": [".*VIBRA.*"] }, "drum": true, "pan": "0" },
            { "id": "WHISTLE", "midi": [{ "name": "Short Whistle", "key_based": true }], "subst": { "id1": "MARACAS", "id2": "RIMSHOT", "id3": "HIT" }, "name": { "syn": [".*WHISTL.*"] }, "drum": true, "pan": "0" }
        ]
    };
