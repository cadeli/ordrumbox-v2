...
export default class InstrumentsManager {
    static DATA = {
        "instruments": [
            ...
            { "id": "SYNTHPAD", "midi": [{ "name": "Pad 1 (new age)", "key_based": false }], "subst": { "id1": "HIT", "id2": "CRASH", "id3": "RIMSHOT" }, "name": { "syn": [".*SYNTH.*PAD.*", ".*PAD.*", ".*NEW.*AGE.*", ".*WARM.*", ".*BOWED.*", ".*METALLIC.*", ".*HALO.*", ".*SWEEP.*"] }, "drum": true, "pan": "0" },
