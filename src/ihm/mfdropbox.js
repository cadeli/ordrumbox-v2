...
export default class MfDropBox {
  ...
  setSelectedItemNum = (itemNum, prefix) => { //TODO
    if (!Array.isArray(this.lastItemList) || this.lastItemList.length === 0) {
      return
    }
    const selectedItem = this.lastItemList[itemNum]
    if (!selectedItem) {
      return
    }
    this.fillDropBox(this.lastItemList, selectedItem, prefix ?? this.lastPrefix)
  }
  ...
  fillDropBox = (itemList, selectedItem, prefix) => {
    this.lastItemList = [...itemList]
    this.lastPrefix = prefix
    Utils.recursiveClear(this.itemlistdiv)
    if (!selectedItem) {
