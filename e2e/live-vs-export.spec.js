...
test.describe('Live vs Export equivalence', () => {
    ...
    test('WavExporter and direct engine render produce equivalent audio for the loaded pattern', async ({ page }) => {
        ...
        expect(
            rmsDiff,
            `RMS difference: export=${result.exportRms.toFixed(4)} direct=${result.directRms.toFixed(4)}`,
        ).toBeLessThan(0.05)

        const peakDiff =
            Math.abs(result.exportPeak - result.directPeak) / Math.max(result.exportPeak, result.directPeak)
        expect(
            peakDiff,
            `Peak difference: export=${result.exportPeak.toFixed(4)} direct=${result.directPeak.toFixed(4)}`,
        ).toBeLessThan(0.05)
                ...
                tracks: [
                    {
                        ...
                        velocity: 1,
                        notes: [
                            { beat: 0, beatStep: 0, pitch: 0 },
                            { beat: 0, beatStep: 8, pitch: 0 },
                        ],
                    },
                ],
            }
