...
function main() {
    ...
    for (const { inst, wavPath } of wavFiles) {
        ...
        const noteStr = midiChanged
            ? `${oldMidi}(${oldNote}) -> ${inst.rootMidi}(${midiToNoteName(inst.rootMidi)})`
            : `${inst.rootMidi}(${oldNote}) [same]`
        console.log(
            `  [${tag}] ${path.relative(KITS_DIR, wavPath).padEnd(30)} peak: ${oldPeakDb.toFixed(1)} -> ${newPeakDb.toFixed(1)} dB  note: ${noteStr}  freq: ${freq || '-'} Hz  conf: ${confidence}`,
        )

        if (midiChanged) {
            noteChanges.push({
                file: path.relative(KITS_DIR, wavPath),
                from: `${oldMidi}(${oldNote})`,
                to: `${inst.rootMidi}(${midiToNoteName(inst.rootMidi)})`,
                freq,
            })
        }
    }
