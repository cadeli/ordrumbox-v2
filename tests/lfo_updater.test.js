...
describe('LfoUpdater', () => {
    describe('computeLfoValue', () => {
        it('returns 0 when lfo is null', () => {
            expect(computeLfoValue(null, 100, 128)).toBe(0)
        })

        it('returns 0 when lfo is undefined', () => {
            expect(computeLfoValue(undefined, 100, 128)).toBe(0)
        })
        ...
        it('returns a number within [min, max] range', () => {
            ...
            for (let tick = 0; tick < 200; tick += 10) {
                const val = computeLfoValue(lfo, tick, baseTicks)
        ...
        it('uses default ticksPer4Bars (TICK * 4) when not provided', () => {
            ...
            const withDefault = computeLfoValue(lfo, 0)
            const withExplicit = computeLfoValue(lfo, 0, TICK * 4)
            expect(withDefault).toBe(withExplicit)
        })

        it('returns value rounded to 2 decimal places', () => {
            const lfo = { freq: 2, phase: 0.25, min: 0.1, max: 0.9 }
            const val = computeLfoValue(lfo, 50, TICK * 4)
        ...
        it('phase offset shifts the waveform', () => {
            ...
            const valA = computeLfoValue(lfoA, 40, TICK * 4)
            const valB = computeLfoValue(lfoB, 40, TICK * 4)
        ...
        it('min=max returns min value throughout', () => {
            ...
            const val = computeLfoValue(lfo, 100, TICK * 4)
