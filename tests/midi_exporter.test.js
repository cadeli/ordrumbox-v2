...
function makeNote(bar, barStep, velocity = 0.8, pitch = 0) {
    return { bar, barStep, velocity, pitch }
}
...
describe('resolveTrackMidi', () => {
    ...
    it('resolves HTOM to MIDI note 50', () => {
        expect(resolveTrackMidi('HTOM', im).midiNote).toBe(50)
    })

    it('resolves LTOM to MIDI note 45', () => {
        expect(resolveTrackMidi('LTOM', im).midiNote).toBe(45)
    })
...
describe('buildInstrumentTrack', () => {
    ...
    it('second note in bar 1, barStep 0 has correct absolute tick offset', () => {
        const track = makeTrack('KICK', [makeNote(1, 0, 1.0)], { barQuantize: 4 })
        const chunk = buildInstrumentTrack(track, 36, 9)
        const events = parseMTrkEvents(chunk, 8, chunk.length - 8)
        const on = events.find(e => e.type === 'midi' && e.status === 0x90)
...
describe('MidiExporter', () => {
    ...
    it('pattern with default bpm/nbBars is exportable', () => {
