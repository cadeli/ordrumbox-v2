 ...
 * well past 50 ms for the 64-beat / 8-track pattern, the JS bookkeeping of
...
function buildHeavyPattern() {
    ...
    for (let i = 0; i < NB_TRACKS; i++) {
        const notes = []
        for (let beat = 0; beat < NB_BEATS; beat++) {
            for (let step = 0; step < STEPS_PER_BEAT; step++) {
                if ((step + i) % 4 === 0) {
                    notes.push({ beat, beatStep: step, velocity: 0.8 })
                }
            }
        }
        tracks.push({
            ...
            loopLength: NB_BEATS, loopEnabled: false,
        })
    }
    return {
...
describe('Worklet pipeline JS cost (proxy benchmark)', () => {
    ...
    it('measure ensureLoaded + strip creation + mixer.start + syncAllTracks', async () => {
        ...
        console.log(`Pattern      : ${NB_BEATS} beats × ${NB_TRACKS} tracks × ${STEPS_PER_BEAT} steps`)
