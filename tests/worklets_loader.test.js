...
describe('WorkletLoader', () => {
    ...
    it('registers a processor and reports it via has/list', () => {
        ...
        expect(WorkletLoader.has('beat')).toBe(false)
    ...
    // Regression test for a data race: several call sites (strip creation,
    // the synth-voice pool, the master bus, ...) can call ensureLoaded()
    // for the same context without awaiting each other — e.g. several
    // notes of a chord triggering at once. Before the fix, each concurrent
    // call would see the processor as "not yet loaded" and start its own
    // addModule(), which — per the Web Audio spec — throws on the second
    // registerProcessor() call for the same name in one
    // AudioWorkletGlobalScope, silently dropping whatever note/node
    ...
    it('ensureLoaded() deduplicates concurrent calls for the same context (no duplicate addModule)', async () => {
        ...
        WorkletLoader.register('synth-voice', 'src')

        // Five "notes" call ensureLoaded() at once, none awaiting the
        // others — exactly what a chord firing on session start does.
        const calls = [
            WorkletLoader.ensureLoaded(fakeCtx),
            WorkletLoader.ensureLoaded(fakeCtx),
            WorkletLoader.ensureLoaded(fakeCtx),
            WorkletLoader.ensureLoaded(fakeCtx),
            WorkletLoader.ensureLoaded(fakeCtx),
    ...
    it('ensureLoaded() allows a fresh load after a failed one (no stuck pending state)', async () => {
        ...
        await expect(WorkletLoader.ensureLoaded(fakeCtx)).rejects.toThrow('boom')
        await expect(WorkletLoader.ensureLoaded(fakeCtx)).resolves.toBe(true)
