...
function makeNote(bar, barStep, props = {}) {
    return { bar, barStep, velocity: 0.8, pitch: 0, ...props }
}
...
describe('MfPatterns', () => {
    ...
    describe('computeNextPatternStepNote', () => {
        it('finds next note in same bar', () => {
        ...
        it('finds next note in next bar', () => {
            const track = makeTrack({
                barQuantize: 4,
                bars: 2,
        ...
        it('wraps around bars correctly', () => {
            const track = makeTrack({
                barQuantize: 4,
                bars: 3,
    ...
    describe('generateSubNotes', () => {
        ...
        it('generates retrigger notes when retriggerNum > 1', () => {
            const flatNotes = new Map()
            const track = makeTrack({ barQuantize: 4 })
            const note = makeNote(0, 0, { retriggerNum: 3, retriggerStep: 1 })
        ...
        it('does not exceed nbTickForPattern', () => {
            const flatNotes = new Map()
            const track = makeTrack({ barQuantize: 4 })
            const note = makeNote(0, 0, { retriggerNum: 8, retriggerStep: 1 })
