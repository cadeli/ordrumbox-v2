...
function makeTrack(overrides = {}) {
    return {
        name: 'KICK',
        velocity: 0.8, pan: 0, pitch: 0, sampleLength: 1,
        ...
        soundId: '', bars: 4, barQuantize: 4, loopAtStep: 16, swingAmount: 0,
        ...overrides,
    }
}
...
describe('TrackEditor — OrSlider integration', () => {
    ...
    it('booleans stay as native buttons (not sliders)', () => {
        editor._track = makeTrack()
        editor.sync()
        const muteBtn = editor.container.querySelector('button[data-key="mute"]')
        expect(muteBtn).not.toBeNull()
        expect(editor.container.querySelector('input[type=range][data-key="mute"]')).toBeNull()
    })
    ...
    it('selects stay as native <select> (not sliders)', () => {
        ...
        editor._track = makeTrack({
            reverbAmount: 0.5, delayAmount: 0.3, saturationAmount: 0.2,
        })
        editor.sync()
