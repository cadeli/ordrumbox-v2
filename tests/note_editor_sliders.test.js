...
async function showNote(ne, overrides = {}) {
    const note = {
        ...
        triggerFreq: 1, triggerPhase: 0, triggerProbability: 1,
        euclidianFill: 0,
        retriggerNum: 1, retriggerStep: 1,
        arpRange: 0, arpTriggerProbability: 1,
        ...overrides,
    }
    const track = { name: 'SNARE', notes: [note], bars: 1, barQuantize: 4 }
    await ne.show({ track, note, pos: 0, bar: 0, barStep: 0 })
    return { note, track }
}
...
describe('NoteEditor — OrSlider integration', () => {
    ...
    it('renders a range slider for each trigger/retrig/arpRange prop', async () => {
        ...
        for (const key of [
            'triggerFreq', 'triggerPhase', 'triggerProbability', 'euclidianFill',
            'retriggerNum', 'retriggerStep',
