...
describe('MfCmd: extended coverage', () => {
    ...
    describe('euclidianFill', () => {
        it('adds notes between startStep and endStep', () => {
            const track = mfCmd.createTrack(4, 'KICK', 4)
            mfCmd.euclidianFill(track, 0, 16, 4, 1)
            // 16 / 4 = 4 steps spacing, notes at 4, 8, 12
            expect(track.notes.length).toBe(3)
            expect(track.notes[0].bar).toBe(1)
            expect(track.notes[0].barStep).toBe(0)
            expect(track.notes[1].bar).toBe(2)
            expect(track.notes[1].barStep).toBe(0)
            expect(track.notes[2].bar).toBe(3)
            expect(track.notes[2].barStep).toBe(0)
        })

        it('does nothing when internalStep <= 0', () => {
            const track = mfCmd.createTrack(4, 'KICK', 4)
            mfCmd.euclidianFill(track, 0, 2, 4, 1)
            expect(track.notes.length).toBe(0)
        })
    })

    describe('compacteTrack', () => {
        it('compacts four-on-floor to loopAtStep=1', () => {
            const track = mfCmd.createTrack(4, 'KICK', 4)
            for (let bar = 0; bar < 4; bar++) {
                for (let step = 0; step < 4; step++) {
                    mfCmd.addNote(track, bar, step)
                }
            }

            mfCmd.compacteTrack(track)

            expect(track.loopAtStep).toBe(1)
        })

        it('compacts half-note pattern to loopAtStep=2', () => {
            const track = mfCmd.createTrack(4, 'KICK', 4)
            for (let bar = 0; bar < 4; bar++) {
                mfCmd.addNote(track, bar, 0)
                mfCmd.addNote(track, bar, 2)
            }
        ...
        it('compacts quarter-note pattern to loopAtStep=4', () => {
            const track = mfCmd.createTrack(4, 'KICK', 4)
            for (let bar = 0; bar < 4; bar++) {
                mfCmd.addNote(track, bar, 0)
            }
        ...
        it('compacts 2-bar repeating pattern', () => {
            const track = mfCmd.createTrack(4, 'KICK', 4)
            for (let bar = 0; bar < 2; bar++) {
                mfCmd.addNote(track, bar, 0)
                mfCmd.addNote(track, bar, 1)
            }
    ...
    describe('compareTrack', () => {
        it('returns true for identical track names and note counts', () => {
            const track1 = mfCmd.createTrack(4, 'KICK', 4)
            const track2 = mfCmd.createTrack(4, 'KICK', 4)
            track1.notes = [{ name: 'n1' }, { name: 'n2' }]
            track2.notes = [{ name: 'n1' }, { name: 'n2' }]

            expect(mfCmd.compareTrack(track1, track2)).toBe(true)
        })

        it('returns false for different note lengths', () => {
            const track1 = mfCmd.createTrack(4, 'KICK', 4)
            const track2 = mfCmd.createTrack(4, 'KICK', 4)
            track1.notes = [{ name: 'n1' }]
            track2.notes = [{ name: 'n1' }, { name: 'n2' }]

            expect(mfCmd.compareTrack(track1, track2)).toBe(false)
        })

        it('returns false for different track names', () => {
            const track1 = mfCmd.createTrack(4, 'KICK', 4)
            const track2 = mfCmd.createTrack(4, 'SNARE', 4)

            expect(mfCmd.compareTrack(track1, track2)).toBe(false)
        })

        it('returns false for different note names', () => {
            const track1 = mfCmd.createTrack(4, 'KICK', 4)
            const track2 = mfCmd.createTrack(4, 'KICK', 4)
            track1.notes = [{ name: 'n1' }]
            track2.notes = [{ name: 'n2' }]
